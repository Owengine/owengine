...
class INSTALL_CTRL_Install extends INSTALL_ActionController
{
    ...
    {
        $this->setPageHeading("Welcome to Owengine Installation!");
        ...
        foreach ( $rules as $ruleLine )
        {
			...
			//05:07 p.m. 11/05/2017 Adding support for alternatives ext1|ext2|ext3 (ext1 or ext2 or ext3)
                        case 'extensionsAlternatives':
                            $requiredExtensions = array_map('trim', explode(',', $value));
                            $loadedExtensions = get_loaded_extensions();							
							foreach ($requiredExtensions as $var => $val){
								$thisExtensions = explode('|', $val);
								$diff = array_values(array_diff($thisExtensions, $loadedExtensions));								
								if ( count($diff) == count($thisExtensions) )
								{
									$fails['php'][$phpOption] = $val;
								}	
							}
                            break;
