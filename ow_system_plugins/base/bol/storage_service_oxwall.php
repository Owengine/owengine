 ...
 * @since 1.8.1
 */
class BOL_StorageServiceOxwall
{
    ...
    public function getStorageUrl( $uri )
    {
        return self::UPDATE_SERVER . UTIL_String::removeFirstAndLastSlashes($uri) . "/";
    }

    public function triggerEventBeforeRequest( $params = array() )
    {
    ...
    public function updateItemsUpdateStatus( array $items )
    {
        if ( empty($items) )
        {
            return;
    ...
    public function updateItemsLicenseStatus( array $items )
    {
    ...
    public function notifyAdminAboutInvalidItems( array $items )
    {
        if ( empty($items) )
        {
            return;
    ...
    public function requestGetResultAsJson( $url, $data )
    {
